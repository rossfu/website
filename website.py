...
# 1. Cache the SentenceTransformer embedder
@st.cache_resource
def load_embedder():
    return SentenceTransformer("all-MiniLM-L6-v2")

# 2. Cache the resume text chunks and FAISS index
@st.cache_data
def load_resume_data(embedder):
...
# 3. Cache the language model pipeline
@st.cache_resource
def load_llm():
...
if not st.session_state.model_loaded:
    if st.button("Yes!"):
        with st.spinner("Loading model and embedding resume..."):
            ...
            st.session_state.chunks, st.session_state.index = load_resume_data(st.session_state.embedder)
            st.session_state.rag_model = load_llm()
            st.session_state.model_loaded = True
        st.experimental_rerun()

# 6. User input after model is ready
